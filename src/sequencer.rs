...
pub enum SeqCommand {
    CreateMidiInput(DeviceId, PortMidiDeviceId), //create the specified portMidiInput with the specified id
...
pub fn sequencer_process(receiver: Receiver<MusitCommand>) {
	...
	let mut pattern__connection_list: HashMap<DeviceId, PatternId> = HashMap::new();
			        	...
			        	match comm {
			        		SeqCommand::CreateMidiInput(id, device_id) => {
			        			...
			        			if let None = input_device_list.iter().find(|&d| d.id == id)	{
			        				match PortMidiInputDevice::new(device_id as DeviceId, device_id) {
			        					Ok(new_dev) => {
						    ...
						    SeqCommand::ConnectProcess(device_id, process) => {
						    	if let Some(input) = input_device_list.iter().find(|&d| d.id == device_id)	{
...
//	    pattern_list.iter_mut().filter(|&(_,ref p)| p.receiver.is_some()).map(|(_,p)| p.try_read_channel()).collect::<Vec<()>>();
		for input in input_device_list.iter() {
			...
			for event  in event_list {
				...
				if pattern__connection_list.contains_key(&input.id) {
				}
